...
def app():
    ...
    if study:
        ...
        screened_studies = cur.execute('SELECT COUNT(*) FROM studies WHERE Decision IS NOT NULL').fetchone()[0]
        ...
        if col1.button('Include'):
            cur.execute('UPDATE studies SET Decision = ? WHERE id = ?', ('Include', study[0]))
            conn.commit()
        if col2.button('Exclude'):
            cur.execute('UPDATE studies SET Decision = ? WHERE id = ?', ('Exclude', study[0]))
            conn.commit()
        if col3.button('Next'):
        ...
        included_count = cur.execute('SELECT COUNT(*) FROM studies WHERE Decision = "Include"').fetchone()[0]
        excluded_count = cur.execute('SELECT COUNT(*) FROM studies WHERE Decision = "Exclude"').fetchone()[0]
        undecided_count = cur.execute('SELECT COUNT(*) FROM studies WHERE Decision IS NULL').fetchone()[0]
